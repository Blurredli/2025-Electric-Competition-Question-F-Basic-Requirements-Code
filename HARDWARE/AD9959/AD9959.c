...
//	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOA|RCC_APB2Periph_GPIOC, ENABLE);	 //使能PB,PE端口时钟
  __HAL_RCC_GPIOB_CLK_ENABLE(); //开启 GPIOB时钟
	__HAL_RCC_GPIOC_CLK_ENABLE(); //开启 GPIOC时钟
		__HAL_RCC_GPIOF_CLK_ENABLE(); //开启 GPIOF时钟
	
	
	GPIO_InitStructure.Pin = GPIO_PIN_6|GPIO_PIN_8|GPIO_PIN_7;//初始化管脚PF6.7.8
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP; 		 //推挽输出
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_VERY_HIGH ;		 //IO口速度为50MHz
	HAL_GPIO_Init(GPIOF, &GPIO_InitStructure);					 //根据设定参数初始化GPIOF
	
	GPIO_InitStructure.Pin =GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_12|GPIO_PIN_13;//初始化管脚PB12,13,2,3,4,5    
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP; 		 //推挽输出
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_VERY_HIGH;		 //IO口速度为50MHz
	HAL_GPIO_Init(GPIOB, &GPIO_InitStructure);					 //根据设定参数初始化GPIOB
	
	GPIO_InitStructure.Pin = GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_10;//初始化管脚PB0.1.10			
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP; 		 //推挽输出
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_LOW;		 //IO口速度为2MHz
	HAL_GPIO_Init(GPIOB, &GPIO_InitStructure);					 //根据设定参数初始化GPIOB
	
	GPIO_InitStructure.Pin = GPIO_PIN_0;	//初始化管脚PC0			 
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP; 		 //推挽输出
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_LOW;		 //IO口速度为2MHz
	HAL_GPIO_Init(GPIOC, &GPIO_InitStructure);					 //根据设定参数初始化GPIOC
